...
if __name__ == "__main__":
    ...
    for idx in range(1, 12):
        ...
        df_log = pd.read_csv(f'device_logs/device_records_expriment2.csv', sep=',')
        weight_list = pd.read_csv(f'device_logs/output_weights.csv', sep=',')['Weight'].tolist()
        ...
        # 计算trust_value
        df_log['trust'] = df_log['dtm'] * weight_list[0] + df_log['rtm'] * weight_list[1] + df_log['rsm'] * weight_list[2]
    ...
    # 保存为 CSV 文件
    result_df.to_csv('output_F1_experiment2.csv', index=False, header=True)
    print(f1)
