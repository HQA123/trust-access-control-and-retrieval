...
# 读取CSV文件，指定分隔符为分号
df = pd.read_csv(r'C:\Users\user\Desktop\新建文件夹\SIoT-IoT-Dataset-main\adjacency_matricies\SOR2.csv', sep=';', header=None)
...
for idx in range(1,12):
    ...
    df_copy = df_copy.drop(drop_by_index) # 删除行
    print("drop row successfully!")
    df_copy = df_copy.drop(df_copy.columns[drop_by_index], axis=1) # 删除列
    print("drop col successfully!")

    # 将DataFrame保存到CSV文件，不包括索引
    df_copy.to_csv('output_SOR2.csv', index=False, header=False)
