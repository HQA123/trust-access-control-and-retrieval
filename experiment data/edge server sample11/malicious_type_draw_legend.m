% 加载CSV文件
data = readtable('aligned_results.csv');
...
for i = 1:length(column_names)
    ...
    if i<= 3
        ...
        plot(time_points(sparse_indices), device_mean(sparse_indices), ...
             ...
             'MarkerSize', 5, 'DisplayName', column_names{i});  
    elseif i<=7
        plot(time_points, device_mean, 'Color', colors(i, :), ...
             'LineWidth', 1.5, 'HandleVisibility', 'off');
        % 稀疏显示标记点（每隔30个点显示一个），不添加到图例中
        ...
        plot(time_points(sparse_indices), device_mean(sparse_indices), ...
             ...
             'MarkerSize', 6, 'DisplayName', column_names{i});  
    elseif i<=11
        plot(time_points, device_mean, 'Color', colors(i, :), ...
             'LineWidth', 1.5, 'HandleVisibility', 'off');
        % 稀疏显示标记点（每隔30个点显示一个），不添加到图例中
        ...
        plot(time_points(sparse_indices), device_mean(sparse_indices), ...
             '-*', 'Color', colors(i, :), 'MarkerFaceColor', colors(i, :), ...
             'MarkerSize', 7, 'DisplayName', column_names{i});  
    end
end
